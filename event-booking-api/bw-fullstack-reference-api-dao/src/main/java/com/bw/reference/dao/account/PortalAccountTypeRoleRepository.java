...
public interface PortalAccountTypeRoleRepository extends JpaRepository<PortalAccountTypeRole, Long>, QuerydslPredicateExecutor<PortalAccountTypeRole> {
    ...
    Optional<PortalAccountTypeRole> findFirstByAccountTypeAndNameAndStatus(PortalAccountTypeConstant portalAccountType, String name, GenericStatusConstant status);
    ...
    @Query("select r from PortalAccountTypeRole r where r.id in " +
            "(select p.role.id from PortalAccountMemberRole p where p.membership.workspaceUser.userId=?1 and p.status='ACTIVE') " +
            "and r.status=?2")
